# gui/tab_editor.py
"""
Editor tab interface
"""

import tkinter as tk
from tkinter import ttk, filedialog, messagebox
import logging
import os
from typing import Dict, Any, Optional

class EditorTab:
    """
    Editor tab interface for video processing
    """
    
    def __init__(self, parent, theme_manager, config: Dict[str, Any] = None):
        self.parent = parent
        self.theme_manager = theme_manager
        self.config = config or {}
        self.logger = logging.getLogger(__name__)
        
        # Create main frame
        self.frame = self.theme_manager.create_custom_frame(parent, padding=10)
        self.frame.grid_columnconfigure(0, weight=1)

        # Setup UI components
        self.setup_input_section()
        self.setup_timeline_section()
        self.setup_effects_section()
        self.setup_process_section()
        
        self.logger.info("EditorTab initialized")
    
    def setup_input_section(self):
        """Setup input file/URL section using .grid()"""
        input_frame = self.theme_manager.create_custom_frame(self.frame, padding=5)
        input_frame.grid(row=0, column=0, sticky='ew', pady=(0, 10))
        input_frame.grid_columnconfigure(1, weight=1)

        # Title
        title_label = self.theme_manager.create_custom_label(
            input_frame, "Input Source", font=('Arial', 12, 'bold'))
        title_label.grid(row=0, column=0, columnspan=2, sticky='w')
        
        # Button frame
        button_frame = self.theme_manager.create_custom_frame(input_frame)
        button_frame.grid(row=1, column=0, columnspan=2, sticky='w', pady=5)
        
        self.local_file_btn = self.theme_manager.create_custom_button(
            button_frame, "Local File", command=self.load_local_file)
        self.local_file_btn.grid(row=0, column=0, padx=(0, 5))
        
        self.url_btn = self.theme_manager.create_custom_button(
            button_frame, "URL Import", command=self.show_url_dialog)
        self.url_btn.grid(row=0, column=1, padx=(0, 5))

        # File and Quality Frame
        file_quality_frame = self.theme_manager.create_custom_frame(input_frame)
        file_quality_frame.grid(row=2, column=0, columnspan=2, sticky='ew', pady=5)
        quality_label = self.theme_manager.create_custom_label(file_quality_frame, "Quality:")
        quality_label.grid(row=0, column=0, sticky='w')
        
        self.quality_var = tk.StringVar(value="720p")
        quality_combo = self.theme_manager.create_custom_combobox(
            file_quality_frame, values=["360p", "480p", "720p", "1080p"], width=10)
        quality_combo.set("720p")
        quality_combo.grid(row=0, column=1, sticky='w')

        self.file_label = self.theme_manager.create_custom_label(
            input_frame, "No file selected", font=('Arial', 10, 'italic'))
        self.file_label.grid(row=3, column=0, columnspan=2, sticky='w', pady=5)

        # URL entry (hidden by default)
        self.url_frame = self.theme_manager.create_custom_frame(input_frame)
        self.url_frame.grid(row=4, column=0, columnspan=2, sticky='ew', pady=5)
        self.url_frame.grid_remove() # Hide it initially
        
        url_label = self.theme_manager.create_custom_label(self.url_frame, "URL:")
        url_label.grid(row=0, column=0, padx=(0, 5))
        self.url_entry = self.theme_manager.create_custom_entry(self.url_frame)
        self.url_entry.grid(row=0, column=1, sticky='ew')
        self.url_frame.grid_columnconfigure(1, weight=1)
        
        self.download_btn = self.theme_manager.create_custom_button(
            self.url_frame, "Download", command=self.download_url)
        self.download_btn.grid(row=0, column=2, padx=(5, 0))

    def setup_timeline_section(self):
        """Setup timeline controls section using .grid()"""
        timeline_frame = self.theme_manager.create_custom_frame(self.frame, padding=5)
        timeline_frame.grid(row=1, column=0, sticky='ew', pady=(0, 10))
        timeline_frame.grid_columnconfigure(1, weight=1)

        title_label = self.theme_manager.create_custom_label(
            timeline_frame, "Timeline Controls", font=('Arial', 12, 'bold'))
        title_label.grid(row=0, column=0, columnspan=3, sticky='w')
        
        # Trim intro
        intro_label = self.theme_manager.create_custom_label(timeline_frame, "Trim Intro:")
        intro_label.grid(row=1, column=0, sticky='w', pady=5)
        self.intro_var = tk.DoubleVar(value=0.0)
        intro_scale = self.theme_manager.create_custom_scale(
            timeline_frame, from_=0, to=60, variable=self.intro_var, orient='horizontal')
        intro_scale.grid(row=1, column=1, sticky='ew', padx=5)
        self.intro_time_label = self.theme_manager.create_custom_label(timeline_frame, "00:00")
        self.intro_time_label.grid(row=1, column=2, sticky='w')
        
        # Trim outro
        outro_label = self.theme_manager.create_custom_label(timeline_frame, "Trim Outro:")
        outro_label.grid(row=2, column=0, sticky='w', pady=5)
        self.outro_var = tk.DoubleVar(value=180.0)
        outro_scale = self.theme_manager.create_custom_scale(
            timeline_frame, from_=0, to=300, variable=self.outro_var, orient='horizontal')
        outro_scale.grid(row=2, column=1, sticky='ew', padx=5)
        self.outro_time_label = self.theme_manager.create_custom_label(timeline_frame, "03:00")
        self.outro_time_label.grid(row=2, column=2, sticky='w')
        
        # Silent threshold
        silent_label = self.theme_manager.create_custom_label(timeline_frame, "Silent Threshold:")
        silent_label.grid(row=3, column=0, sticky='w', pady=5)
        self.threshold_var = tk.DoubleVar(value=-40.0)
        threshold_scale = self.theme_manager.create_custom_scale(
            timeline_frame, from_=-60, to=-20, variable=self.threshold_var, orient='horizontal')
        threshold_scale.grid(row=3, column=1, sticky='ew', padx=5)
        self.threshold_label = self.theme_manager.create_custom_label(timeline_frame, "-40dB")
        self.threshold_label.grid(row=3, column=2, sticky='w')

    def setup_effects_section(self):
        """Setup effects section using .grid()"""
        effects_frame = self.theme_manager.create_custom_frame(self.frame, padding=5)
        effects_frame.grid(row=2, column=0, sticky='ew', pady=(0, 10))
        effects_frame.grid_columnconfigure(1, weight=1)

        title_label = self.theme_manager.create_custom_label(
            effects_frame, "Effects", font=('Arial', 12, 'bold'))
        title_label.grid(row=0, column=0, columnspan=2, sticky='w')
        
        # Selective blur
        blur_label = self.theme_manager.create_custom_label(effects_frame, "Selective Blur:")
        blur_label.grid(row=1, column=0, sticky='w', pady=5)
        blur_button_frame = self.theme_manager.create_custom_frame(effects_frame)
        blur_button_frame.grid(row=1, column=1, sticky='w')
        self.add_region_btn = self.theme_manager.create_custom_button(
            blur_button_frame, "Add Region", command=self.add_blur_region)
        self.add_region_btn.grid(row=0, column=0, padx=(0, 5))
        self.clear_regions_btn = self.theme_manager.create_custom_button(
            blur_button_frame, "Clear All", command=self.clear_blur_regions)
        self.clear_regions_btn.grid(row=0, column=1)
        
        # Color preset
        color_label = self.theme_manager.create_custom_label(effects_frame, "Color Preset:")
        color_label.grid(row=2, column=0, sticky='w', pady=5)
        self.preset_var = tk.StringVar(value="Cinematic")
        preset_combo = self.theme_manager.create_custom_combobox(
            effects_frame, values=["Cinematic", "Vibrant", "Dramatic", "Natural", "B&W"], width=15)
        preset_combo.set("Cinematic")
        preset_combo.grid(row=2, column=1, sticky='w')
        
        # Scene detection
        scene_frame = self.theme_manager.create_custom_frame(effects_frame)
        scene_frame.grid(row=3, column=1, sticky='w', pady=5)
        self.scene_var = tk.BooleanVar(value=True)
        scene_check = self.theme_manager.create_custom_checkbutton(
            effects_frame, "Scene Detection", variable=self.scene_var)
        scene_check.grid(row=3, column=0, sticky='w')
        
        sensitivity_label = self.theme_manager.create_custom_label(scene_frame, "Sensitivity:")
        sensitivity_label.grid(row=0, column=0, sticky='w')
        self.sensitivity_var = tk.DoubleVar(value=0.3)
        sensitivity_scale = self.theme_manager.create_custom_scale(
            scene_frame, from_=0.1, to=0.8, variable=self.sensitivity_var, orient='horizontal')
        sensitivity_scale.grid(row=0, column=1, sticky='ew')
        scene_frame.grid_columnconfigure(1, weight=1)

    def setup_process_section(self):
        """Setup process controls section using .grid()"""
        process_frame = self.theme_manager.create_custom_frame(self.frame, padding=5)
        process_frame.grid(row=3, column=0, sticky='ew', pady=(0, 10))

        title_label = self.theme_manager.create_custom_label(
            process_frame, "Processing", font=('Arial', 12, 'bold'))
        title_label.grid(row=0, column=0, columnspan=2, sticky='w')
        
        # Button frame
        button_frame = self.theme_manager.create_custom_frame(process_frame)
        button_frame.grid(row=1, column=0, columnspan=2, sticky='w', pady=5)
        self.preview_btn = self.theme_manager.create_custom_button(
            button_frame, "SMART PREVIEW", command=self.generate_preview, width=20)
        self.preview_btn.grid(row=0, column=0, padx=(0, 5))
        self.process_btn = self.theme_manager.create_custom_button(
            button_frame, "START PROCESSING", command=self.start_processing, width=20)
        self.process_btn.grid(row=0, column=1)
        
        # Options frame
        options_frame = self.theme_manager.create_custom_frame(process_frame)
        options_frame.grid(row=2, column=0, columnspan=2, sticky='w', pady=5)
        self.fair_use_var = tk.BooleanVar(value=True)
        fair_use_check = self.theme_manager.create_custom_checkbutton(
            options_frame, "Fair Use Mode", variable=self.fair_use_var)
        fair_use_check.grid(row=0, column=0, padx=(0, 10))
        self.auto_effects_var = tk.BooleanVar(value=True)
        auto_effects_check = self.theme_manager.create_custom_checkbutton(
            options_frame, "Auto Effects", variable=self.auto_effects_var)
        auto_effects_check.grid(row=0, column=1)

    def load_local_file(self):
        """Load local video file"""
        filetypes = [("Video files", "*.mp4 *.avi *.mov *.mkv *.wmv"), ("All files", "*.*")]
        filename = filedialog.askopenfilename(title="Open Video File", filetypes=filetypes)
        if filename:
            self.current_file = filename
            self.file_label.config(text=f"Loaded: {os.path.basename(filename)}")
            self.logger.info(f"Loaded local file: {filename}")
    
    def show_url_dialog(self):
        """Show URL input dialog"""
        if self.url_frame.winfo_viewable():
            self.url_frame.grid_remove()
        else:
            self.url_frame.grid()
    
    def download_url(self):
        """Download video from URL"""
        url = self.url_entry.get().strip()
        if not url:
            messagebox.showwarning("No URL", "Please enter a URL")
            return
        self.logger.info(f"Downloading from URL: {url}")
        messagebox.showinfo("Download", f"Downloading from: {url}")
    
    def add_blur_region(self):
        """Add blur region"""
        self.logger.info("Add blur region clicked")
        messagebox.showinfo("Blur Region", "Click and drag on video to select blur region")
    
    def clear_blur_regions(self):
        """Clear all blur regions"""
        self.logger.info("Clear blur regions clicked")
        messagebox.showinfo("Clear Regions", "All blur regions cleared")
    
    def generate_preview(self):
        """Generate smart preview"""
        if not hasattr(self, 'current_file') or not self.current_file:
            messagebox.showwarning("No File", "Please load a video file first")
            return
        self.logger.info("Generating preview")
        messagebox.showinfo("Preview", "Generating smart preview...")
    
    def start_processing(self):
        """Start video processing"""
        if not hasattr(self, 'current_file') or not self.current_file:
            messagebox.showwarning("No File", "Please load a video file first")
            return
        self.logger.info("Starting processing")
        messagebox.showinfo("Processing", "Starting video processing...")
    
    def update_time_labels(self):
        """Update time display labels"""
        intro_time = self.intro_var.get()
        minutes, seconds = int(intro_time // 60), int(intro_time % 60)
        self.intro_time_label.config(text=f"{minutes:02d}:{seconds:02d}")
        
        outro_time = self.outro_var.get()
        minutes, seconds = int(outro_time // 60), int(outro_time % 60)
        self.outro_time_label.config(text=f"{minutes:02d}:{seconds:02d}")
        
        threshold = self.threshold_var.get()
        self.threshold_label.config(text=f"{threshold:.0f}dB")
        
        self.frame.after(100, self.update_time_labels)

# Test the editor tab
if __name__ == "__main__":
    import logging
    import os
    
    logging.basicConfig(level=logging.INFO)
    
    root = tk.Tk()
    root.title("Editor Tab Test")
    root.geometry("800x800")
    
    # Mock theme manager
    class MockThemeManager:
        def create_custom_frame(self, parent, style='TFrame', padding=0):
            return ttk.Frame(parent, padding=padding)
        
        def create_custom_label(self, parent, text, font=None):
            return ttk.Label(parent, text=text, font=font)
        
        def create_custom_button(self, parent, text, command=None, width=None):
            return ttk.Button(parent, text=text, command=command, width=width)
        
        def create_custom_entry(self, parent, width=None):
            return ttk.Entry(parent, width=width)
        
        def create_custom_combobox(self, parent, values=None, width=None):
            return ttk.Combobox(parent, values=values, width=width)
        
        def create_custom_scale(self, parent, from_=0, to=100, variable=None, orient='horizontal', length=None):
            return ttk.Scale(parent, from_=from_, to=to, variable=variable, orient=orient)
        
        def create_custom_checkbutton(self, parent, text, variable=None):
            return ttk.Checkbutton(parent, text=text, variable=variable)
    
    theme_manager = MockThemeManager()
    editor_tab = EditorTab(root, theme_manager)
    editor_tab.frame.pack(fill='both', expand=True) # Use pack here for simple test window
    
    editor_tab.update_time_labels()
    
    root.mainloop()
